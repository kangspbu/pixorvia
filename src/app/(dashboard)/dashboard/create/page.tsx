...
const Page = ({}) => {
  ...
  const [errorTransformingMsg, setErrorTransformingMsg] = useState<
    string | null
  >(null);
  ...
  const getImageKitAuth = async () => {
    ...
    return response.json() as Promise<UploadAuthResponseType>;
  };
  ...
  const handleUploadFile = async (
    ...
    try {
      ...
      //save project to db
      const projectResult = await createProjectInDb({
      ...
      if (projectResult.success) {
        ...
        if (updatedProjects.success && updatedProjects.data) {
          setUserProjects(updatedProjects.data);
        }
      } else {
        //handle error
        console.error(
          "Failed to save project to our database",
          projectResult.error,
        );
      }
  ...
  //to get live preview transformation
  ...
  const removeBackground = async () => {
    ...
    if (hasTransformation("background")) {
      ...
      setErrorTransformingMsg("Background removal is already applied!");
      return;
    }
    ...
    try {
      const deductCreditsResult = await deductCredits(2, "background removal");

      if (!deductCreditsResult.success) {
        toast.error(
          deductCreditsResult.error ?? "Failed to deduct credits. Try again.",
        );
      ...
      //   setTimeout(() => {
      //     // setIsLoadingProcessing(false);
      //     toast.success("Background removal applied!, credit deducted by 2");
      //     //refresh the credits in sidebar
      //     router.refresh();
      //   }, 3000);
    } catch (error) {
      console.error("Error removing background:", error);
      toast.error("Failed to remove background. Try again.");
      setErrorTransformingMsg("Failed to remove background. Try again.");
  ...
  const upscaleImage = async () => {
    ...
    try {
      ...
      setErrorTransformingMsg("Failed to upscale image");
      setIsLoadingProcessing(false);
    }
  };
  ...
  const objectCrop = async () => {
    ...
    try {
      ...
      const cleanInput = objectName.trim().toLowerCase();
      ...
      //   setTimeout(() => {
      //     // setIsLoadingProcessing(false);
      //     toast.success(`Smart crop applied focusing on "${objectName}"!`);
      //   }, 3000);
    } catch (error) {
      console.error("Object crop error:", error);
      toast.error("Failed to apply smart crop");
      setErrorTransformingMsg("Failed to apply smart crop");
                      ...
                      <div className="bg-muted relative overflow-hidden rounded-lg border">
                        ...
                        <ImageKitImage
                          ...
                          onLoadingComplete={() => {
                            ...
                            if (errorTransformingMsg) {
                              toast.error(errorTransformingMsg);
                            }

                            setErrorTransformingMsg(null);
                          }}
